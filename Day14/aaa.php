<?php
    header('Content-Type:text/htm1; charset=stf-8');

    //사용자가 get방식으로 전달된 값을 변수에 저장
    $nickname= $_GET['nickname'];
    $email= $_GET['email'];

    //실제로는 데이터베이스에 저장하는 작업 수행하고 결과를 응답(response - echo)
    //db작업까지 하면 시간이 오래 걸리니 지금은 그냥 받은 데이터를 응답해주기
    echo "$nickname - $email 값으로 회원가입을 했습니다."

    //ajax 기술이 없을 때 페이지가 변경되었다는 것을 사용자가 인식하지 못하도록
    //응답할 때 원래 웹 페이지를 그대로 다시 출력
    echo ("
    <!DOCTYPE html>
    <html lang='ko'>
    <head>
    ...
    <body>
        ...
        <hr>
        <textarea cols='50' rows='5'>$nickname - $email 데이터로 회워가입했습니다.</textarea>
